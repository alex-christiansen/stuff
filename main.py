...
def action_execute(request):
    ...
    url = request['scheduled_plan']['url']

    # q_num = url.find('?')
    # filter_string = url[q_num+1:]
    ...
    #     equal_sign = i.find('=')
    #     filter_name = i[0:equal_sign]    
    #     print(filter_name.replace('+', ' ')+': '+i[equal_sign+1:].replace('+', ' '))
    #     filter_string[a] = filter_name.replace('+', ' ')+': '+i[equal_sign+1:].replace('+', ' ')
    #     a +=  1

    # filters = '\n'.join(filter_string)
    # print('Filters: ', filters)
    # data = io.StringIO(filters)
    # df_filter = pd.read_csv(data)
    ...
    # df_filter.to_excel(writer, sheet_name='Cover Sheet')
